...
public class LeitorCSV {
    ...
    private static final NumberFormat numberFormat = NumberFormat.getInstance(Locale.forLanguageTag("pt-BR"));
    ...
    public static List<Docente> lerDocentes(String caminho) throws IOException {
        ...
        for (String linha : linhas) {
            ...
            LocalDate dataIngresso = LocalDate.parse(campos[3].trim(), formatter);
    ...
    public static List<Ocorrencia> lerOcorrencias(String caminho) throws IOException {
        ...
        for (String linha : linhas) {
            ...
            LocalDate dataInicio = LocalDate.parse(campos[2].trim(), formatter);
            LocalDate dataFim = LocalDate.parse(campos[3].trim(), formatter);
    ...
    public static List<Veiculo> lerVeiculos(String caminho) throws IOException {
        ...
        for (String linha : linhas) {
            ...
            try {
                fatorImpacto = numberFormat.parse(campos[3].trim()).doubleValue();
    ...
    public static List<Regra> lerRegras(String caminho) throws IOException {
        ...
        for (String linha : linhas) {
            String[] campos = linha.split(";");
            LocalDate dataInicio = LocalDate.parse(campos[0].trim(), formatter);
            LocalDate dataFim = LocalDate.parse(campos[1].trim(), formatter);
            ...
            for (int i = 0; i < qualis1.length; i++) {
                try {
                    qualisPontos.put(qualis1[i].trim(), numberFormat.parse(pontos[i].trim()).doubleValue());
                } catch (ParseException e) {
                    e.printStackTrace();
                }
            }
            ...
            try {
                pontuacaoMinima = numberFormat.parse(campos[8].trim()).doubleValue();
            } catch (ParseException e) {
                e.printStackTrace();
            }
